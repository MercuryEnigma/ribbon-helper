...
export { computeGen7Speed as computeModernSpeed }
...
export type ModernMoveDex = Record<string, {
  bp: number
  type: string
  category?: string
  isSpread?: boolean
...
export type ModernPokedex = Record<string, PokedexEntry>
export type ModernSetdex = Record<string, Record<string, SetdexEntry>>
...
function makeFieldSide(overrides: Partial<FieldSide> = {}, format = 'singles', weather = '', terrain = ''): FieldSide {
  return {
...
function checkAirLock(pokemon: Pokemon, fieldState: { weather: string; clearWeather(): void }): void {
  if (pokemon.curAbility === 'Air Lock' || pokemon.curAbility === 'Cloud Nine') {
    fieldState.clearWeather()
  }
}

function checkKlutz(pokemon: Pokemon): void {
...
export function createModernBattleCalc(data: {
  moves: ModernMoveDex
  pokedex: ModernPokedex
  setdex: ModernSetdex
}) {
  const { moves, pokedex, setdex } = data
  ...
  function findSetByLabel(label: string): { species: string; set: SetdexEntry } | null {
  ...
  function resolveSpeciesName(name: string): string {
    return pokedex[name] ? name : name
  }
