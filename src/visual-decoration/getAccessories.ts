...
type PageGroup = Record<string, Acquisition[]>;
...
export function getAccessories(theme: Theme, eligibleAcquisitions: Set<Acquisition>): AccessoryResult {
  ...
  Object.entries(accessories as Record<string, AccessoryRecord>).forEach(([id, data]) => {
    const acquisitions = data.acquisition as Acquisition[];
    const hasEligible = acquisitions.some((a) => eligibleAcquisitions.has(a));
    ...
    result[score][pageLabel][id] = acquisitions;
  });
